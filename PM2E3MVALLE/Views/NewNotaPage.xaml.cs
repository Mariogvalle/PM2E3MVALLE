...
{
    FirebaseClient client = new FirebaseClient("https://pm2e3mvalle-default-rtdb.firebaseio.com/");
    //control para audio
...
///        _audioRecorder = AudioManager.Current.CreateRecorder();
    ...
    private async void seleccionarButton_Clicked(object sender, EventArgs e)
    {
        ...
        if (foto != null)
        {
            ...
            UrlFoto = await new FirebaseStorage("pm2e3mvalle.appspot.com")
                            .Child("Photos")
    ...
    private async Task StopRecordingAsync()
    {
        try
        {
            ...
            {
                UrlAudio = await new FirebaseStorage("pm2e3mvalle.appspot.com")
                    .Child("Audios")
